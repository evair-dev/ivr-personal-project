...
from ivr_gateway.models.workflows import Workflow, WorkflowConfig, WorkflowRun
from tests.factories import queues as qf
from ivr_gateway.steps.api.v1 import InputActionStep, NoopStep, BranchMapWorkflowStep, PlayMessageStep
from ivr_gateway.steps.config import StepTree, StepBranch, Step
from tests.factories import workflow as wcf


class TestTaggingWorkflowConfigsOnCallRoutings:
    ...
    @pytest.fixture
    def initial_step_tree(self) -> StepTree:
        return StepTree(
            branches=[
                StepBranch(
                    name="root",
    ...
    @pytest.fixture
    def workflow(self, db_session: SQLAlchemySession, initial_step_tree: StepTree, modified_step_tree: StepTree)\
            -> Workflow:
        workflow_factory = wcf.workflow_factory(db_session, "main_menu", step_tree=initial_step_tree)
        ...
        db_session.commit()
        # Now alter the workflow so that the latest is a new tree
    ...
    def test_new_call(self, db_session, workflow, greeting, call_routing, Iivr_unsecured_any_queue, Iivr_any_queue,
                      test_client):
        ...
        assert first_call_second_response.data == b'<?xml version="1.0" encoding="UTF-8"?><Response><Dial>773-695-2581</Dial></Re' \
                                                  b'sponse>'
        contact = db_session.query(Contact).filter(Contact.global_id == "twilio:test1").first()
        first_workflow_run: WorkflowRun = contact.contact_legs[0].workflow_run
        ...
        fetched_workflow = db_session.query(Workflow).filter(Workflow.id == workflow.id).first()
        assert first_workflow_run.workflow_config.step_tree_sha != fetched_workflow.latest_config.step_tree_sha
